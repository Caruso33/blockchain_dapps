...
function displayAirlines(airlines, contract) {
  ...
  headerRow.appendChild(
    DOM.div({ className: "col-sm-6 field" }, "Airline address")
  )
  headerRow.appendChild(
    DOM.div({ className: "col-sm-2 field" }, "Sponsoring airline")
  )
  headerRow.appendChild(
    DOM.div({ className: "col-sm-2 field" }, "Registration")
  )
  ...
  airlines.map((airline) => {
    ...
    row.appendChild(
      DOM.div({ className: "col-sm-6 field text-truncate" }, airline)
    )
    ...
    contract.isAirlineRegistered(airline, (error, result) => {
      ...
      if (error) {
        ...
        registrationButton = DOM.button(
          { className: "col btn table-button", disabled: true },
          registrationText
        )
      } else if (result) {
        ...
        registrationButton = DOM.button(
          { className: "col btn btn-primary table-button", value: airline },
          registrationText
        )
        registrationButton.addEventListener("click", function (event) {
          registerAirline(event, contract, airlines)
        })
      }
    ...
    contract.isAirlineActivated(airline, (error, result) => {
      ...
      if (error) {
        ...
        activationButton.addEventListener("click", function (event) {
          activateAirline(event, contract, airlines)
        })
      }
